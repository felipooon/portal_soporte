...
async function procesarArchivosEvidencias(files) {
  ...
  for (let file of files) {
    ...
    reader.onload = async (e) => {
      ...
      try {
        const res = await fetch("/certificado/api/upload_evidencia", {
          method: "POST",
...
async function procesarArchivoAlarmas(file) {
  ...
  reader.onload = async (e) => {
    const base64 = e.target.result;
    try {
      const res = await fetch("/certificado/api/upload_alarmas", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre: file.name, base64: base64, location: location })
      });
...
async function procesarPegadoTextoAlarmas() {
  ...
  try {
    const res = await fetch("/certificado/api/parse_alarmas_texto", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function cargarListaCertificadosHeader(autoLoadFirst = false) {
  ...
  try {
    const res = await fetch("/certificado/api/list?año=2026");
    const data = await res.json();
...
async function cargarCertificadoPorLocation(locationId) {
  try {
    const res = await fetch("/certificado/api/load", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function eliminarCertificadoPorLocation(locationId) {
  try {
    const res = await fetch("/certificado/api/delete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ location: locationId, año: 2026 })
    });
...
async function guardarAvance() {
  ...
  try {
    const res = await fetch("/certificado/api/save", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ certificado: certificadoState })
    });
...
async function ejecutarIngresoTecnico() {
  ...
  try {
    const response = await fetch("/certificado/api/revisor/ingreso_tecnico", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        host,
...
async function generarPlantillaIngreso({ notificar = false } = {}) {
  ...
  try {
    const response = await fetch("/certificado/api/revisor/generar_plantilla_ingreso", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        dns: host,
